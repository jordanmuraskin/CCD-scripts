...
def getSubjectButtonResponses():
    ...
    for indx,f in enumerate(filelist['JM_INTERNAL']):
        for r in range(1,3):
            ...
            out['filename']=f
            if (indx+r)==1:
                dmn_df=pd.DataFrame(out,index=[0])
                dmn_df['subject']=f
                dmn_df['run']=r
            else:
                ...
                tmp['subject']=f
                tmp['run']=r
                dmn_df=pd.concat((dmn_df,tmp),ignore_index=0)
    return dmn_df
...
dmn_subject_list = np.unique(dmn_df['Subject'])

#Create the different groups
focus_and_wander=np.intersect1d(dmn_df[np.all([dmn_df.fb=='FEEDBACK',dmn_df.cope==3,dmn_df.signal<0],axis=0)]['Subject'],dmn_df[np.all([dmn_df.fb=='FEEDBACK',dmn_df.cope==4,dmn_df.signal>0],axis=0)]['Subject'])
focus_only=np.intersect1d(dmn_df[np.all([dmn_df.fb=='FEEDBACK',dmn_df.cope==3,dmn_df.signal<0],axis=0)]['Subject'],dmn_df[np.all([dmn_df.fb=='FEEDBACK',dmn_df.cope==4,dmn_df.signal<0],axis=0)]['Subject'])
wander_only=np.intersect1d(dmn_df[np.all([dmn_df.fb=='FEEDBACK',dmn_df.cope==3,dmn_df.signal>0],axis=0)]['Subject'],dmn_df[np.all([dmn_df.fb=='FEEDBACK',dmn_df.cope==4,dmn_df.signal>0],axis=0)]['Subject'])
...
if runAll==1:
    ...
    dmn_df=getSubjectButtonResponses()
    tmp=dmn_df.groupby('subject')['number'].sum()
...
secondlevel_folder_names=['noFeedback','Feedback','train','NFB+FB']

subject_list = np.intersect1d(subject_list,dmn_subject_list)
...
if run1Sample:
    ...
    for i in copesToRun:
        for fb in fbtorun:
            ...
            ('F-Test', 'F',[('W>F+W', 'T',['wander_only','focus_and_wander'],[1,-1]),('F>F+W', 'T',['focus_only','focus_and_wander'],[1,-1])])]
            if age:
            ...
            if runWithRandomise:
                filename='%scope%d_cov_anova' % (prefix,i)
                if age:
                    filename+='_age'
                if gender:
                    filename+='_gender'
                ...
                randomiseCommand='/home/jmuraskin/Projects/CCD/CCD-scripts/analysis/randomise_forpython.sh -i %s/%s -o ./%s/cope%d -d ./%s/design.mat -t ./%s/design.con -e ./%s/design.grp -f ./%s/design.fts -m %s -T -n %d' % (filename,'cope' + str(i) + '_merged.nii.gz',filename,i,filename,filename,filename,filename,mask_name,nperms)
